...
def decode(temp_dir, compressed_file, FLAGS, len_series, last, bpe_ckpt):
    bs = FLAGS.batch_size
...
def encode(temp_dir, compressed_file, FLAGS, series, train_data, last_train_data):
    ...
    for train_index in range(iter_num):
        ...
        if train_index % FLAGS.print_step == 0:
            ...
            print(train_index, ":", train_loss.item() / np.log(2), "size:", size)
...
def main(_):
    ...
    if (len_series - FLAGS.seq_len) % FLAGS.batch_size == 0:
        decode(temp_dir, compressed_file, FLAGS, len_series, 0, bpe_ckpt=prefix)
    else:
        last_length = (len_series - FLAGS.seq_len) % FLAGS.batch_size + FLAGS.seq_len
        decode(temp_dir, compressed_file, FLAGS, len_series, last_length, bpe_ckpt=prefix)
