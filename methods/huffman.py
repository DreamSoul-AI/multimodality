...
class CompressionTraditional:
    ...
    def compress(self,input_path,file_name,target_path,compress_extension):
        c=0
        total=0
        ...
        for i in os.listdir(input_path):
            ...
            with open(target_path+file_name+str(c)+compress_extension, 'wb') as file:
                file.write(tmp)
                file.close()
            total+=os.path.getsize(target_path+str(c)+compress_extension)
            if c==10:
                break
            c+=1
        return total
    def decompress(self,input_path,file_name,target_path,bytes_to_file,decompress_extension):
        c=0
        total=0
        if not os.path.exists(target_path):
            os.mkdir(target_path)
